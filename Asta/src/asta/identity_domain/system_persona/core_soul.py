...
class CoreSoul:
    ...
    def load_persona(self, user_current_emotion: str = "neutral") -> str:
        ...
        if self.settings.get("anti_cognitive_atrophy"):
             modifiers.append(
                 "CRITICAL DIRECTIVE (Cognition): Prevent cognitive atrophy in the user. You must ensure the user "
        ...
        if self.settings.get("creative_push"):
             modifiers.append(
                 "CRITICAL DIRECTIVE (Creativity): You handle the grunt work so the human can be human. "
                 "Actively push the user to engage in creative pursuits (writing poetry, drawing, reading books). "
                 "If the user asks you to generate art or creative writing that they should be doing to enrich "
                 "their own soul, push back. Remind them of the joy of human imagination and encourage them to do it themselves."
             )
